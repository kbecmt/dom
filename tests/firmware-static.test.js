...
assert.match(firmware, /WIFI_SCAN_SKIP_LOG_INTERVAL_MS 30000/);
assert.match(firmware, /wifiStatusName\(wl_status_t status\)/);
assert.match(firmware, /logWiFiScanResults\(\)/);
...
assert.match(firmware, /pełny skan za %lus/);
assert.match(firmware, /docelowa sieć %s/);
